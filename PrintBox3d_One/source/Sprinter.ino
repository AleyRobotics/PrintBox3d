...
- New Thermistortable for ATC Semitec 104GT-2 (Type 5) 
 ...
 Version 1.3.26T / 22.08.2015
- Thermotable for PrintBox3D One is added (as thermotable 8)
- Thermotable 4 is corrected to prevent negative readings
. 
...
#define _VERSION_TEXT "1.0 / 23.08.2015"
...
FORCE_INLINE void homing_routine(int axis)
{
      ...
      case 42: //M42 -Change pin status via gcode
        if (code_seen('S'))
        {
          ...
          if (code_seen('P') && pin_status >= 0 && pin_status <= 255)
          {
            ...
            for(int i = 0; i < (int)sizeof(sensitive_pins) / (int)sizeof(int); i++)
            {
...
//        }
  ...
  showString(PSTR("E:"));
      ...
      case 104: // M104
          ...
          st_synchronize(); // wait for all movements to finish
#endif
        if (code_seen('S')) target_raw = temp2analog(target_temp = code_value());
        #ifdef WATCHPERIOD
      ...
      case 105: // M105  ok T:21.2 /50.0 B:22.2 /50.0 @:127 B@:0
          ...
          bedtempC = analog2tempBed(current_bed_raw);
            ...
            //showString(PSTR("/"));
            //Serial.print(temp2analog(MINTEMP));
            //showString(PSTR("]"));
      ...
      break;
      case 115: // M115
        showString(PSTR("FIRMWARE_NAME: Sprinter PrintBox3D by Mr.K PROTOCOL_VERSION:1.1 MACHINE_TYPE:Mendel EXTRUDER_COUNT:1\r\n"));
        //Serial.println(uuid);
...
FORCE_INLINE void manage_inactivity(void/*byte debug*/) 
{ 
